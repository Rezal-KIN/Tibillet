module "gala" {
  source   = "./modules/gala"
  for_each = var.enable_additive_resources ? var.galas : {}
  ...
  backup_bucket_arn    = var.enable_backup_storage ? aws_s3_bucket.backups[0].arn : null
  backup_bucket_name   = var.enable_backup_storage ? aws_s3_bucket.backups[0].bucket : null
