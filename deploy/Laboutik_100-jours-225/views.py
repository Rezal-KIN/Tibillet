        ...
        try:
            ...
            try:
                ...
                carte, created = CarteCashless.objects.get_or_create(
                    ...
                    defaults={
                        'number': tag_id_request,
                        'uuid_qrcode': uuid.uuid4(),
                    }
                )
                if created:
                    logger.info(f"Carte vierge créée dans Laboutik : {tag_id_request}")
                elif not carte.uuid_qrcode:
                    carte.uuid_qrcode = uuid.uuid4()
                    carte.number = carte.number or tag_id_request
                    carte.save()
