    ...
    def handle(self, *args, **options):
        class Install(object):
            ...
            def _base_config(self, options):
                ...
                # Never silently repoint an already paired cashless database.
                # The shared public hostnames are stable across Galas, but a
                # different configured target still requires explicit review.
                lespass_url = os.environ['LESPASS_TENANT_URL']
                fedow_url = os.environ['FEDOW_URL']
                if config.string_connect and config.billetterie_url and config.billetterie_url.rstrip("/") != lespass_url.rstrip("/"):
                    raise Exception("Existing Lespass pairing targets another URL")
                if config.can_fedow() and config.fedow_domain and config.fedow_domain.rstrip("/") != fedow_url.rstrip("/"):
                    raise Exception("Existing Fedow pairing targets another URL")
            ...
            def _create_admin_from_env_email(self):
                ...
                if created:
                    try:
                        ...
                        logger.exception("Email for admin activation FAILED")
                call_command('check_permissions')
                return admin

            def _lespass_handshake(self):
                # The Lespass pairing is single-use. A deploy can be retried after
                # that pairing but before the remaining catalogue/admin setup.
                # Resume from the durable local handshake instead of polling a
                # Lespass endpoint which correctly returns 405 once paired.
                config = Configuration.get_solo()
                lespass_url = os.environ['LESPASS_TENANT_URL']
                if config.string_connect and config.key_billetterie:
                    if config.billetterie_url.rstrip("/") != lespass_url.rstrip("/"):
                        raise Exception("Existing Lespass pairing targets another URL")
                    logger.info("Existing Lespass pairing found; resuming install")
                    return
                lespass_state = None
                ping_count = 0
            ...
            def _fedow_handshake(self):
                # A second deployment must not repeat an already successful
                # handshake or create a second Fedow place.
                config = Configuration.get_solo()
                fedow_url = os.environ['FEDOW_URL']
                if config.can_fedow():
                    if config.fedow_domain.rstrip("/") != fedow_url.rstrip("/"):
                        raise Exception("Existing Fedow pairing targets another URL")
                    logger.info("Existing Fedow pairing found; resuming install")
                    return
