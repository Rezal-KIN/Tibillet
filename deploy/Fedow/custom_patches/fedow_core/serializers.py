...
class WalletGetOrCreate(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if FedowUser.objects.filter(email=email).exists():
            ...
            if not self.user.wallet:
                self.user.wallet = wallet_creator(
                    ip=get_request_ip(request),
...
class TransactionW2W(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if action == Transaction.REFILL:
            if not Transaction.objects.filter(asset=self.asset).exists():
                logger.warning(
                    f"{timezone.localtime()} WARNING NewTransactionWallet2WalletValidator : "
                    f"missing FIRST block for asset {self.asset.uuid} -> auto create"
                )
                Transaction.objects.create(
                    ip="0.0.0.0",
                    sender=self.asset.wallet_origin,
                    receiver=self.asset.wallet_origin,
                    asset=self.asset,
                    amount=0,
                    datetime=self.asset.created_at if self.asset.created_at else timezone.localtime(),
