...
def connexion(request):
    if request.method == 'POST':
        ...
        if validator.is_valid():
            ...
            # Nouveau flow front: étape 1 = check email existant
            if step == 'check':
                user_exists = TibilletUser.objects.filter(email__iexact=email).exists()
                if user_exists:
                    ...
                    if settings.TEST or settings.DEBUG:
                        ...
                        test_url = f"https://{base_url}/emailconfirmation/{token}"

                    return JsonResponse({
                        "ok": True,
                        "exists": True,
                        "mail_sent": True,
                        "message": _("Login link sent. Please check your email inbox and spam folder."),
                        "test_url": test_url,
                    })
                ...
                return JsonResponse({
                    ...
                    "exists": False,
                    "mail_sent": False,
                    "message": _("Email not found. Please complete your registration."),
                })

            # Nouveau flow front: étape 2 = inscription avec nom/prénom + envoi du lien
            if step == 'finalize':
                ...
                user = get_or_create_user(email=email, send_mail=False, force_mail=False)
                if not user:
                    return JsonResponse({
                        "ok": False,
                        "error": "EMAIL_ERROR",
                ...
                # Envoi du lien après finalisation
                get_or_create_user(email=email, send_mail=True, force_mail=True)
                ...
                return JsonResponse({
                    ...
                    "exists": True,
                    "mail_sent": True,
                    "message": _("Registration completed. Login link sent by email."),
                })

            # Fallback: comportement historique (POST classique)
            user = get_or_create_user(email=email, send_mail=True, force_mail=True)
            if not user:
                messages.add_message(request, messages.WARNING, _("Invalid email address."))
                return redirect('index')
        ...
        if request.POST.get('step') in ['check', 'finalize']:
            return JsonResponse({
                "ok": False,
                "error": "INVALID_EMAIL",
                "message": _("Invalid email address.")
...
class ScanQrCode(viewsets.ViewSet):  # /qr
    ...
    def _extract_claim_qrcode_uuid(self, request):
        """
        Supporte 3 formats d'URL pour les cartes imprimées :
        - /qr/claim/?qrcode_uuid=<uuid>
        - /qr/claim/?uid=<8HEX>
        - /qr/claim/?token=<signed payload> (payload avec qrcode_uuid ou uid)
        """
        fedowAPI = FedowAPI()

        qrcode_uuid = (request.GET.get('qrcode_uuid') or "").strip()
        uid = (request.GET.get('uid') or "").strip().upper()
        token = (request.GET.get('token') or "").strip()

        if token:
            try:
                payload = signing.loads(token, max_age=365 * 24 * 3600)
                if isinstance(payload, dict):
                    qrcode_uuid = (payload.get("qrcode_uuid") or qrcode_uuid).strip()
                    uid = (payload.get("uid") or uid).strip().upper()
            except Exception as e:
                logger.warning(f"claim token invalide: {e}")
                raise Http404("Token invalide")

        if qrcode_uuid:
            try:
                uuid.UUID(qrcode_uuid)
                return qrcode_uuid
            except Exception:
                raise Http404("qrcode_uuid invalide")

        if uid:
            if not re.match(r'^[0-9A-F]{8}$', uid):
                raise Http404("uid invalide")
            serialized_card = fedowAPI.NFCcard.card_number_retrieve(uid)
            if not serialized_card:
                raise Http404("Carte inconnue")
            qrcode_uuid = serialized_card.get('qrcode_uuid')
            if not qrcode_uuid:
                raise Http404("Carte sans qrcode")
            return qrcode_uuid

        raise Http404("Aucun identifiant carte trouvé")

    def _link_claim_card_to_user(self, request, qrcode_uuid):
        ...
        card_data = fedowAPI.NFCcard.qr_retrieve(qrcode_uuid)
        if not card_data:
            raise Http404("Carte inconnue")

        if not card_data.get('is_wallet_ephemere', False):
            # Carte déjà liée : tolérance si c'est déjà la carte de l'utilisateur courant.
            try:
                existing_wallet_uuid = card_data.get('wallet_uuid')
                if existing_wallet_uuid and user.wallet and str(user.wallet.uuid) == str(existing_wallet_uuid):
                    messages.add_message(request, messages.SUCCESS, _("This card is already linked to your account."))
                    return True
            except Exception:
                pass
            messages.add_message(request, messages.ERROR, _("This card is already linked to another account."))
            return False

        retrieve_wallet = fedowAPI.wallet.retrieve_by_signature(user)
        if retrieve_wallet and retrieve_wallet.validated_data.get('has_user_card'):
            messages.add_message(
                request,
                messages.ERROR,
                _("You already have a linked card. Revoke it first in your profile to link a new one.")
            )
            return False
        ...
        if not linked_serialized_card:
            messages.add_message(request, messages.ERROR, _("Unable to link this card."))
            return False

        # Rattache d'éventuelles adhésions faites avant liaison.
        card_number = linked_serialized_card.get('number_printed')
        if card_number:
            for membership in Membership.objects.filter(user__isnull=True, card_number=card_number):
        ...
        logger.info(
            f"CARD CLAIM LINKED : user={user.email} wallet={user.wallet.uuid if user.wallet else None} qrcode={qrcode_uuid}"
        )
        messages.add_message(request, messages.SUCCESS, _("Your card has been linked to your account."))
        return True
    ...
    def claim(self, request):
        """
        Flow auto pour cartes imprimées :
        - scan QR -> /qr/claim/?uid=... (ou token signé)
        - si non connecté : redirection login
        - après connexion : liaison automatique carte -> compte
        """
        if not request.user.is_authenticated:
            signed_next_url = signing.dumps(request.get_full_path())
            return redirect(f'/login/login_fullpage?next={signed_next_url}')

        qrcode_uuid = self._extract_claim_qrcode_uuid(request)
        self._link_claim_card_to_user(request, qrcode_uuid)
        return redirect('/my_account/card/')
...
class MyAccount(viewsets.ViewSet):
    ...
    def transactions_table(self, request):
        ...
        for token in tokens:
            names_of_place_federated = []
            if token.get('asset', {}).get('place_origin'):
                place_uuid_origin = token['asset']['place_origin']['uuid']
                place_info = self.get_place_cached_info(place_uuid_origin)
                if place_info:
                    token['asset']['logo'] = place_info.get('logo')
            ...
            for place_federated in token.get('asset', {}).get('place_uuid_federated_with', []):
                place = self.get_place_cached_info(place_federated)
                if place:
                    names_of_place_federated.append(place.get('organisation'))
            token['asset']['names_of_place_federated'] = names_of_place_federated
